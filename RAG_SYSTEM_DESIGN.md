...
- 流程：`search()` 先取稠密 top‑K×候选倍数（默认 30）候选，对候选跑 BM25，将稠密分与 BM25 分各自按最大值相对归一化后加权融合 `final = (1-α)·dense + α·bm25`（`α=rag_hybrid_alpha`，默认 0.45，`hybrid=True` 时启用），再取 top‑K。
  > ⚠️ 融合分是**相对归一化**分（top1 恒≈1.0），与长期记忆的**原始余弦**、剧情层的 **BM25 原始分**量纲不同。`build_rag_context` 的相对门按「分数尺度分组」计算（见 `persona_prompt._score_group`），否则强 lore 命中会把长期记忆整段滤掉。
