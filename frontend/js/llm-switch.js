...
(function () {
  ...
  async function refreshOllamaModels(announce) {
    ...
    try {
      ...
      const data = await resp.json();
      // 拉取期间用户可能已切到云端 provider：复查后再改写下拉，
      // 否则云端面板仍会被本地模型名污染（守卫只在 fetch 前检查是防不住的）。
      if (!providerSel || providerSel.value !== "ollama") return;
  ...
  function saveConfig() {
    ...
    // 持久化切换（后端写 .env + 热重载，QQ 渠道同步生效；api_key 不落盘仅本请求内存使用）
    const wasConfigured = state.configured;
    const btnText = saveBtn ? saveBtn.textContent : "";
    if (saveBtn) { saveBtn.disabled = true; saveBtn.textContent = "⏳ 应用中…"; }
    fetch(API_BASE + "/api/llm/switch", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(f),
    })
      .then((resp) => resp.json().catch(() => ({ ok: false, detail: "HTTP " + resp.status })))
      .then((data) => {
        if (data && data.ok) {
          ...
          state.api_key = f.api_key; // 仅 localStorage；后端不落盘
          saveState();
          close();
          emitChanged();
          toast("🤖", "已切换模型（全局+QQ生效）", getDisplayName());
        } else {
          toast("⚠️", "切换失败", (data && data.detail) || "未知错误");
        }
      })
      .catch((e) => {
