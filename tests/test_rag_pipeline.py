...
from roleplay.core.knowledge.vector_store import _BM25, _tokenize
...
def test_build_roleplay_prompt_request_card_wins_prompt_config_and_tone():
    """请求携带 card_json 时，外层 tone/prompt_config 必须以请求卡为准，
    不得错误地沿用 CharacterStore 当前激活角色的配置（否则直接传卡路径功能失效）。"""
    request_card = CharacterCard(
        name="请求卡",
        personality="冷静",
        tone="请求卡语气",
        prompt_config={"enabled_fields": ["name", "tone", "rag"]},
    )
    store_card = CharacterCard(
        name="库存卡", personality="库存性格", tone="库存语气"
    )
    chunks = [
        RetrievedChunk(
            text="发条装置令大脑自动生成人格。",
            score=0.8,
            metadata={"namespace": "lore_wu_ming_zhe", "section": "设定"},
        )
    ]
    prompt = build_roleplay_prompt(
        card_json=request_card.model_dump_json(),
        fallback_prompt=None,
        default_card=store_card.model_dump_json(),
        message="发条装置是什么？",
        chunks=chunks,
        card=store_card,
    )
    assert "请求卡语气" in prompt
    assert "库存语气" not in prompt
    # 请求卡 prompt_config 关闭了 personality 字段
    assert "冷静" not in prompt
    assert "发条装置" in prompt
...
# ── 混合检索（稠密 + BM25 稀疏重排）──
def test_bm25_tokenize_merges_ascii_words_and_keeps_cjk_chars():
    assert _tokenize("hello world 世界") == ["hello", "world", "世", "界"]
    assert _tokenize("openai gpt-4 模型") == ["openai", "gpt", "4", "模", "型"]
