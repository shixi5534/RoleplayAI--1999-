...
def build_llm(settings: Settings | None = None) -> LLMPort:
    ...
    if s.llm_provider in ("openai", "deepseek", "ollama"):
        ...
        client = OpenAILikeProvider(
            ...
            top_p=s.llm_top_p,
            repetition_penalty=s.llm_repetition_penalty,
        )
...
def build_llm_from_config(
    ...
    简化策略（实际实现）：
    - override 为 None、全字段为 None，或解析后与全局 provider/model/base_url
      完全一致且无 key/timeout → 复用全局实例，needs_aclose=False。
    ...
    # 与全局配置完全一致且不带请求级 key → 直接复用全局缓存实例。
    # 前端 buildOverride() 已做同类判断，但直接调用 API 的客户端可能仍传同值 override，
    # 后端此处兜底可避免每请求新建/销毁 AsyncClient。
    global_base = (s.llm_base_url or _DEFAULT_BASE_URLS.get(s.llm_provider, "")).rstrip("/")
    if (
        not override.api_key
        and override.timeout is None
        and provider == s.llm_provider
        and model == s.llm_model
        and base_url.rstrip("/") == global_base
    ):
        return build_llm(s), False
