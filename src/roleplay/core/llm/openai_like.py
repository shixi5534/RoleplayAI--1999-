...
def _build_messages(
    *,
    ...
    P1-1 分层（两个参数均为可选，缺省时输出与改造前**逐条一致**，零回归）：
    - system_blocks 非空 → 每个块各成一条 system 消息（取代单一 system 串）；
      空白块会被跳过，避免发出空消息。
    - user_prefix 非空 → 拼在当前 user 消息之前（中间空一行），
      把「怎么说」的动态指令放到近因效应最强的位置。
    """
    blocks = [b for b in (system_blocks or []) if isinstance(b, str) and b.strip()]
    if blocks:
        messages: list[dict] = [{"role": "system", "content": b} for b in blocks]
    else:
    ...
    content = f"{user_prefix.strip()}\n\n{user}" if (user_prefix or "").strip() else user
...
class OpenAILikeProvider(LLMPort):
    def __init__(
        ...
        *,
        ...
        # 采样参数：repetition_penalty 用于抑制"复读/照背示例原句"，
        # 是角色扮演场景的关键旋钮（行业共识 1.05-1.15，>1.2 会回避角色名等正常重复）。
        self._top_p = top_p
    ...
    def _apply_sampling(self, payload: dict) -> None:
        """追加采样参数。未配置（None）时保持不传，交由服务端默认。

        注意：Ollama / vLLM 等 OpenAI 兼容端点支持 repetition_penalty，
        标准 OpenAI 接口不识别该字段（会被忽略），因此这里默认可配、可关。
        """
