...
class UserProfile:
    ...
    async def _replace_item(self, item: dict, new_meta: dict) -> None:
        ...
        注意：``KnowledgeBase.aremove`` 会把同步 remove + 文件落盘卸载到线程池；
        ...
        aremove = getattr(self._kb, "aremove", None)
        if aremove is not None:
            await aremove(self._ns, item["id"])
        else:
    ...
    async def _prune_to_max(self) -> int:
        ...
        items = await asyncio.to_thread(self._kb.list_items, self._ns)
        ...
        removed = 0
        aremove = getattr(self._kb, "aremove", None)
        while len(items) - removed > self._max_items and candidates:
            victim = candidates.pop(0)
            ok = (
                await aremove(self._ns, victim["id"])
                if aremove is not None
                else self._kb.remove(self._ns, victim["id"])
            )
            if ok:
                removed += 1
        if removed:
