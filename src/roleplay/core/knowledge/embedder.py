...
class OllamaEmbedder(EmbedderPort):
    ...
    def __init__(self, model: str, base_url: str, timeout: float = 60.0) -> None:
        ...
        # 请求路径复用连接池；同步桥接路径（离线脚本/测试）仍使用独立短连接，
        # 避免 AsyncClient 跨事件循环复用的兼容性风险。
        self._client: httpx.AsyncClient | None = None
        # 记录创建该客户端时的事件循环：httpx 连接池绑定 loop，跨 loop 复用会抛
        # 「Event loop is closed」（多轮 asyncio.run / 每个 TestClient 上下文新建 loop
        # 的场景实测复现）。loop 变更时重建，生产单长驻 loop 行为不变。
    ...
    def _get_client(self) -> httpx.AsyncClient:
        ...
        with self._lock:
            if self._closed:
                raise RuntimeError("OllamaEmbedder 已关闭")
            stale = self._client is not None and (
                (loop is not None and self._client_loop is not loop)
                or (self._client_loop is not None and self._client_loop.is_closed())
            )
            if stale:
                logger.warning(
                    "OllamaEmbedder 检测到事件循环变更/已关闭，重建 HTTP 连接池"
                )
                self._client = None
    ...
    async def _acall_with(
        self, texts: List[str], client: httpx.AsyncClient
    ) -> List[List[float]]:
        """真正的 HTTP 调用，按 _BATCH 分批，避免大批量导入超时。"""
        out: List[List[float]] = []
        for start in range(0, len(texts), self._BATCH):
            batch = texts[start : start + self._BATCH] or [""]
            try:
            ...
            if not isinstance(data, dict):
                raise RuntimeError("Ollama 嵌入响应格式非法")
    ...
    async def _acall(self, texts: List[str]) -> List[List[float]]:
        """请求路径的批量嵌入：复用持久 AsyncClient 连接池。"""
        return await self._acall_with(texts, self._get_client())

    async def _acall_temp(self, texts: List[str]) -> List[List[float]]:
        """同步桥接用：每次使用独立短连接客户端。"""
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            return await self._acall_with(texts, client)

    def _call(self, texts: List[str]) -> List[List[float]]:
        """同步桥接：离线脚本 / 测试用，内部经事件循环运行器调用。"""
        return _run_async(self._acall_temp(texts))
