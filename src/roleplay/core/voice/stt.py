...
# CPU 推理并发上限：Whisper 解码会占满多核，并发请求会互相拖慢并放大内存峰值。
# 使用专属单线程 executor 排队，既避免多事件循环绑定问题，也不占用 asyncio 默认线程池。
_STT_MAX_CONCURRENCY = 1
_STT_EXECUTOR = concurrent.futures.ThreadPoolExecutor(
    max_workers=_STT_MAX_CONCURRENCY, thread_name_prefix="roleplay-stt"
)


async def _run_stt_inference(fn):
    """在受限并发下执行 CPU 密集的转录任务。"""
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(_STT_EXECUTOR, fn)
...
def _filter_hallucinations(text: str) -> str:
    ...
    if (
        ...
        logger.debug("幻觉过滤：整段匹配黑名单，清空 ('%s')", original[:50])
        return ""

    if _RE_REPEAT_PATTERN_EN.match(original):
        logger.debug("幻觉过滤：英文循环模式，清空 ('%s')", original[:50])
        return ""
    ...
    if tail_cleaned != cleaned:
        logger.debug("幻觉过滤：尾部截断 '%s' → '%s'", cleaned[:50], tail_cleaned[:50])
    ...
    if (
        ...
        logger.debug("幻觉过滤：清理后整段匹配黑名单，清空 ('%s')", cleaned[:50])
        return ""

    if _RE_REPEAT_PATTERN_EN.match(cleaned):
        logger.debug("幻觉过滤：清理后英文循环模式，清空 ('%s')", cleaned[:50])
        return ""
...
async def transcribe_audio(
    ...
    try:
        # 首次模型加载是 140MB+ 磁盘/内存操作，必须移出事件循环，避免卡死整个服务
        model = await asyncio.to_thread(
            _get_whisper_model, model_name, model_path_override
        )
    except RuntimeError:
        raise
    ...
    try:
        ...
        text = await _run_stt_inference(_transcribe_and_join)
        # 隐私：转写原文不进日志（与 OCR 的「只记字符数」约定一致），仅 DEBUG 记长度
        logger.debug("STT 识别完成：%d 字符", len(text or ""))
        ...
        logger.debug("STT 幻觉过滤后：%d 字符", len(text or ""))
        return text
