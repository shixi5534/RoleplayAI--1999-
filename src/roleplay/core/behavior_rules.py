...
def match_condition(cond: dict, context: dict[str, Any]) -> bool:
    ...
    - emotion_is：与当前情绪标签相等（None 不命中）；pattern 支持 ``|`` 分隔的
      多值（如 ``sad|fear|anxious``），单值写法语义不变（零回归）。
    """
    ctype = cond.get("type")
    ...
    if ctype == "text_regex":
        try:
            ...
            return False
    if ctype == "emotion_is":
        # 多值：以 | 分隔（"sad|fear|anxious"）；单值与旧版相等判定等价。
        return text in {p.strip() for p in pattern.split("|") if p.strip()}
    return False
