...
def _llm_layer_available(s: Settings) -> bool:
    """LLM 层是否可用：非 mock，且（有 api_key 或本地 provider）。

    原实现要求 ``api_key`` 非空，导致 **Ollama 部署下 LLM 层静默降级**：
    Ollama 是本地推理、本就不需要 key（factory.py:43 在空 key 时会兜底填 "ollama"），
    于是情感检测退回关键词层，质量无声下降且无任何日志。
    隐私约束的本意是"不私自外发到云端"，本地 provider 不存在外发，故豁免。
    """
