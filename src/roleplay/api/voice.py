...
async def stt_endpoint(
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"detail": f"STT 失败: {e}"})
...
async def tts_endpoint(
    ...
    # 否则 RuntimeError 会延迟到响应流迭代时才抛出，无法转为 500 JSON。
    if not settings.tts_backend_enabled:
        logger.error("TTS 失败: TTS 后端未启用")
        return JSONResponse(
            status_code=500,
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"detail": f"TTS 失败: {e}"})